/* --- ALCHEMIST V101.4 | LOCKED RUNTIME --- */
<script>
    ...
    function injectData() {
        ...
        DATABASE = rawText.split('\n').map(line => {
            try {
                ...
                const qSplit = qAndO.split('?'), question = qSplit[0] + "?", optsRaw = (qSplit[1] || "").trim();

                /* --- THE DEEP RIP LOGIC (FIXES A,B,C PROBLEM) --- */
                const opts = optsRaw
                ...
                return { 
                    ...
                    u: opts[0] || "Option A", r: opts[1] || "Option B", l: opts[2] || "Option C", 
    ...
    function renderDiff() { MODE = "MENU_DIFF"; CUR_INDEX = 0; SCORE = 0; addToLog("SYSTEM_READY", null); renderMenu("SELECT DIFFICULTY", "SET A", "SET B", "SET C", "SET D"); }
    ...
    function renderEnd() {
        ...
        c.innerHTML = `<div class="card-q">COMPLETE<br><span style="color:var(--gold)">${SCORE} XP</span></div><div class="swipe-label sl-up">RESTART</div><div class="swipe-label sl-left">MENU</div><div class="swipe-label sl-right">LEVEL UP</div><div class="swipe-label sl-down">REVIEW</div>`;
        s.appendChild(c); bindPhysics(c, {});
    }

    function bindPhysics(el, data) {
        let x = 0, y = 0, sx = 0, sy = 0, active = false, td = null;
        const lbls = { up: el.querySelector('.sl-up'), dn: el.querySelector('.sl-down'), lt: el.querySelector('.sl-left'), rt: el.querySelector('.sl-right') };
        ...
        window.onmousemove = window.ontouchmove = e => { if (!active) return; const p = e.touches ? e.touches[0] : e; x = p.clientX - sx; y = p.clientY - sy; el.style.transform = `translate3d(${x}px,${y}px,0) rotate(${x/20}deg)`; const ang = Math.atan2(-y, x) * (180 / Math.PI); Object.values(lbls).forEach(l => { if (l) l.style.opacity = 0; }); if (Math.abs(x) > 20 || Math.abs(y) > 20) { let d = (ang >= 45 && ang < 135) ? "up" : (ang >= 135 || ang < -135) ? "lt" : (ang >= -135 && ang < -45) ? "dn" : "rt"; if (lbls[d]) lbls[d].style.opacity = 1; td = d.toUpperCase(); } };
    ...
    function handleAction(el, data, dir) {
        ...
        else if (MODE === "END") { if (dir === "UP") startQuiz(SESSION_LIMIT); else if (dir === "LT") renderDiff(); else if (dir === "RT") { CUR_DATASET = NEXT_LEVEL[CUR_DATASET] || "SET_A"; startQuiz(SESSION_LIMIT); } else if (dir === "DN") { if (MISTAKES.length > 0) { POOL = [...MISTAKES]; MISTAKES = []; CUR_INDEX = 1; SESSION_LIMIT = POOL.length; MODE = "QUIZ"; renderNext(); } else renderDiff(); } }
