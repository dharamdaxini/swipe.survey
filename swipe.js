...
// 3. DATA IMPORT (RESILIENT PARSER)
async function importDataset() { 
    ...
    for (let line of lines) { 
        let c = line.split(/\t/); 
        if (c.length < 5) c = line.split(/\s{3,}/); // Space-fallback
    ...
    if (parsed.length > 0) {
        ...
        alert("FORMAT ERROR: USE 12-COLUMN TSV");
        btn.disabled = false;
...
function route(el, data, dir) {
    if (MODE === "S_PATH") { SEL_PATH = dir; renderS1(); }
        ...
        if (dir === "DOWN") { 
            if (SEL_PATH === "UP") { resetCard(el); return; } // Test mode lock
            openOverlay(data); resetCard(el); return; 
        }
        if (dir === (data.ans === "UP" ? "UP" : data.ans === "LEFT" ? "LEFT" : "RIGHT")) SCORE += 10;
...
// 6. MODAL & HUD
function resetCard(el) { el.style.transition = "0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)"; el.style.transform = "none"; }
...
function renderEnd() { document.getElementById('stack').innerHTML = `<div class='card'><div class='card-q'>MISSION COMPLETE</div><div class='swipe-label sl-up' onclick='location.reload()'>RESTART</div></div>`; }
