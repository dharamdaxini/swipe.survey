...
function renderNext() {
    ...
    // SWIPE-DOWN CLEANER
    let cleanEx = q.ex.replace(/[❌✅]/g, '').replace(/\d\.\d+$/, '').trim();
    ...
    c.innerHTML = `
        ...
        <div class="overlay">
            <div class="overlay-meta">${q.id} | ${q.ty}</div>
            <div class="overlay-body">${format(cleanEx)}</div>
            <div class="overlay-hint">> CONCEPT: ${q.tp} Analysis</div>
...
function renderEnd() {
    MODE = "END"; const s = document.getElementById('stack'); s.innerHTML = ""; const c = document.createElement('div');
    c.className = "card";
    c.innerHTML = `<div class="card-q">MISSION COMPLETE<br><span style="color:var(--gold)">${SCORE} XP</span></div><div class="swipe-label sl-up">RESTART</div><div class="swipe-label sl-left">MENU</div><div class="swipe-label sl-right">LEVEL UP</div><div class="swipe-label sl-down">REVIEW</div>`;
...
function bindPhysics(el, data) {
    ...
    window.onmousemove = window.ontouchmove = e => {
        ...
        if (Math.abs(x) > 20 || Math.abs(y) > 20) {
            ...
            if (labels[d]) labels[d].style.opacity = 1; td = d.toUpperCase();
        }
    };
...
function handleAction(el, data, dir) {
    if (MODE === "MENU_DIFF") renderTopic(dir === "UP" ? "SET A" : dir === "LT" ? "SET B" : dir === "RT" ? "SET C" : "SET D");
        ...
        else if (dir === "RT") { CUR_DATASET = NEXT_LEVEL[CUR_DATASET] || "SET_A"; startQuiz(SESSION_LIMIT); }
