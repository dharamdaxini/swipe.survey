...
function initVault() {
    ...
    DATABASE = lines.map(line => {
        try {
            ...
            // ANCHOR PARSER: Rips options from the hard '?' anchor
            const qIndex = qAndO.lastIndexOf('?');
            const question = qAndO.substring(0, qIndex + 1);
            const optsRaw = qAndO.substring(qIndex + 1).trim();
...
/* V81.1 PRECISION PHYSICS ENGINE */
function bindPhysics(el, data) {
    ...
    window.onmousemove = window.ontouchmove = e => { 
        ...
        // V81.1 60FPS Render Loop
        requestAnimationFrame(() => {
            if (active) {
                el.style.transform = `translate3d(${x}px,${y}px,0) rotate(${x/15}deg) scale(1.05)`; 
            }
        });
    ...
    window.onmouseup = window.ontouchend = () => { 
        ...
        if (Math.abs(x) > 110 || Math.abs(y) > 110) handleAction(el, data, td); 
        else { 
